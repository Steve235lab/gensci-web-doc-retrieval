...
* 用户点击 paper_info 页面中的列标，对数据进行排序

    * 前端 -> 后端

        url: 42.192.44.52:8000/search/sort_paper_info/

        ```
        {
            'token': token,
            'message_type': 'sort_paper_info',
            'timestamp': timestamp,
            'column': "Publication_Date" / "Journal_If",
            'order': "positive" / "reverse"
        }
        ```

* 用户点击 clue_info 页面中的列标，对数据进行排序

    * 前端 -> 后端

        url: 42.192.44.52:8000/search/sort_clue_info/

        ```
        {
            'token': token,
            'message_type': 'sort_clue_info',
            'timestamp': timestamp,
            'column': "Weight",
            'order': "positive" / "reverse"
        }
...
* sort_paper_info
    ...
    ```
    {   
        'message_type': 'sorted_paper_info',
        'timestamp': timestamp,
        'paper_info': [paper_0, paper_1, ...],
        'total': total,
        'token': token
